...
class Antylopa(Zwierze):
    def __init__(self, x=0, y=0, sila=4, wiek=0):
    ...
    def ucieka_def(self, plansza, off, def_):
        ...
        print(f"{self.imie} ucieka przed {off.imie} na pole {self.x} {self.y}")
        return off

    def ucieka_off(self, plansza, off, def_):
        plansza[self.x][self.y] = off
        print(f"{self.imie} ucieka przed {def_.imie} na pole {self.x} {self.y}")
        return def_
